...
function search() {
    ...
    let typeName = $("#book_type_name").val();
    let searchName = $("#book_search_name").val();
    $('.card-block table td').remove();
    postBookInformation(typeName, searchName)
}

function postBookInformation(typeName, searchName) {
    $.ajax({
        url: '/PostBookInformation',
        type: 'POST',
        data: {
            bookTypeName: typeName,
            bookSearchName: searchName
        },
        dataType: 'json',
